...
<img src="f1.png" width="1500px" align="center" />
...
You can enter the CPT_prompt folder and run the following code to train and obtain contrastive prompts：
```bash
CUDA_VISIBLE_DEVICES=<DEVICE_ID> python train.py  \
-b ./DATA_ROOT_DIR/train/<origin dataset(e.g. H&E)>/ \
-r ./DATA_ROOT_DIR/train/<target dataset(e.g. MAS)>/ \
--prompt_lr 0.000005                   \
 --eta_min 5e-6                         \
 --weight_decay 0.001                   \
 ...
 --exp_name <your task name>
```
#### Concept anchor
The files generated by different concept anchors are in the VPGAN folder. You can load the corresponding VLM weights and run `python concept.py` (change different concept classes to output the final text embedding (`.pt`)).
